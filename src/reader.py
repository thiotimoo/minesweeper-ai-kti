...
class MinesweeperReader:
    ...
    def load_color_profiles(self):
        """Load saved color profiles, normalize to lists of ints"""
        if os.path.exists('minesweeper_colors.json'):
            try:
                ...
                normalized = {}
                for state, colors in data.items():
                    normalized[state] = []
                    for c in colors:
                        # Ensure it's a list of 3 ints
                        if isinstance(c, (list, tuple)) and len(c) >= 3:
                            normalized[state].append([int(c[0]), int(c[1]), int(c[2])])
                return normalized
            except Exception:
    ...
    def save_color_profiles(self):
        """Save color profiles to file (convert everything to lists of ints first)"""
        safe = {}
        for state, colors in self.color_profiles.items():
            safe[state] = []
            for c in colors:
                # c might be tuple/list or numpy types; convert to plain ints list
                try:
                    safe[state].append([int(c[0]), int(c[1]), int(c[2])])
                except Exception:
        ...
        try:
            ...
            self.log_info("✓ Colors saved successfully")
        except Exception as e:
            self.log_info(f"✗ Error saving colors: {e}")
    ...
    def pick_color(self, state):
        ...
        if color[0]:
            # color[0] is (r, g, b) floats; convert to ints and store as list
            rgb = [int(round(c)) for c in color[0]]
            if state not in self.color_profiles:
                self.color_profiles[state] = []
            # insert as list of ints
            if rgb not in self.color_profiles[state]:
                self.color_profiles[state].insert(0, rgb)
            self.update_color_button(state)
            self.log_info(f"✓ Color set for '{state}': RGB{tuple(rgb)}")
    ...
    def canvas_click(self, event):
        ...
        if 0 <= cell_row < rows and 0 <= cell_col < cols:
            ...
            # Add all sampled colors to the profile (as plain lists of ints)
            ...
            for color in sampled_colors:
                ...
                if rgb not in self.color_profiles[state]:
                    self.color_profiles[state].append(rgb)
            
            self.update_color_button(state)
            
            self.log_info(f"✓ Cell [{cell_row},{cell_col}] colors picked for '{state}':\n   " + 
                         "\n   ".join([f"RGB{tuple(int(c) for c in col)}" for col in sampled_colors]))
    ...
    def reset_colors(self):
        ...
        # remove saved file if exists and reload defaults
        try:
            if os.path.exists('minesweeper_colors.json'):
                os.remove('minesweeper_colors.json')
        except Exception:
            pass
        self.color_profiles = self.load_color_profiles()
    ...
    def get_cell_sample_colors(self, cell):
        ...
        for y, x in sample_points:
            if 0 <= y < h and 0 <= x < w:
                ...
                color = (int(raw[0]), int(raw[1]), int(raw[2])) if len(raw) >= 3 else (int(raw), int(raw), int(raw))
    ...
    def detect_cell_state_multipoint(self, cell):
        ...
        for sample_color in sampled_colors:
            # ensure ints
            sc = (int(sample_color[0]), int(sample_color[1]), int(sample_color[2]))
            for state, ref_colors in self.color_profiles.items():
                for ref_color in ref_colors:
                    rc = (int(ref_color[0]), int(ref_color[1]), int(ref_color[2]))
                    diff = sum(abs(a - b) for a, b in zip(sc, rc))
    ...
    def get_region_info(self):
        ...
        # Most common colors
        color_counts = Counter([tuple(int(c) for c in col) for col in colors])
